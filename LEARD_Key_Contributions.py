    def ResiDense(self, feature, xyzrgb, neigh_idx, d_out, name, is_training):
        ...
        f_pc_2 = self.SAL(f_pc_1, xyzrgb, neigh_idx, d_out // 4, name + 'LFEA_2', is_training)  # B, N, 1, d_out/4
        f_pc_3 = self.SAL(tf.concat([f_pc_1, f_pc_2], axis=-1), xyzrgb, neigh_idx, d_out // 2, name + 'LFEA_3', is_training)  # B, N, 1, d_out/2
    ...
    def LFEA(self, feature, xyzrgb, neigh_idx, d_out, name, is_training):
        ...
        m_BF = helper_tf_util.conv2d(aug_xyzrgb, d_in, [1, 1], name + 'mlp1', [1, 1], 'VALID', True, is_training)
        MF = self.loc_feat(feature, neigh_idx, is_k=False)  # Multidimensional Feature Encoding
        m_MF = helper_tf_util.conv2d(aug_feat, d_in, [1, 1], name + 'mlp2', [1, 1], 'VALID', True, is_training)
        new_BF = helper_tf_util.conv2d(tf.concat([BF, m_MF], axis=-1), d_in, [1, 1], name + 'mlp3', [1, 1], 'VALID', True, is_training)
        new_MF = helper_tf_util.conv2d(tf.concat([MF, m_BF], axis=-1), d_in, [1, 1], name + 'mlp4', [1, 1], 'VALID', True, is_training)
        CF = tf.concat([new_BF, new_MF], axis=-1)  # Cross Feature Encoding

        scope = helper_tf_util.conv2d(CF, d_out // 2, [1, 1], name + 'mlp5', [1, 1], 'VALID', bn=False, activation_fn=None)  # B, N, k, d_out/2
        scope = tf.nn.softmax(scope, axis=2)  # B, N, k, d_out
        attention_pooling = tf.reduce_sum(overall_info * k_weights, axis=2, keepdims=True)  # B, N, 1, d_out/2
